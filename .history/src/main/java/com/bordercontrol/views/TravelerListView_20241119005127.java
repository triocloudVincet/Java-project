...
public class TravelerListView {
    ...
    private static void loadTravelers(TableView<TablePosition> tableView) {
        try {
            TravelerDAO dao = new TravelerDAO();
            ObservableList<Traveler> travelers = FXCollections.observableArrayList(dao.findAll());
            tableView.setItems(travelers);
